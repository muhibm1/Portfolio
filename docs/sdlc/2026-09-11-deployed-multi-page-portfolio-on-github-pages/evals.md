...
| GC89 | R89 | Given the phone number survives in the working tree only where a machine check needs it as a literal, when wave 4 completes, then `docs/sdlc/constraints.md` line 102 and `intent.md` line 199 read `(512) 508-xxxx` with a one-line note that the digits were removed under G1-D1 and where the live check now lives, and a repository-wide grep for the full number, excluding `.git` and `node_modules`, matches `spec.md` only | `grep -rln "508-1536" --exclude-dir=.git --exclude-dir=node_modules .` | Exactly one file matches, `spec.md` | Windows | command, run after wave 4 |
